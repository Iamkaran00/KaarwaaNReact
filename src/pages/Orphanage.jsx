...
import React, { useState, useEffect } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { BookOpen, Users, Droplet, Lightbulb, PenTool } from 'lucide-react';
import childstudying from '../asset/childstudorphanage.jpg';
import orph from '../asset/orph.jpg';
import holi1 from '../asset/holiceleb.jpeg';
import holi2 from '../asset/holiceleb2.jpg';
...
const heroImages = [orph, holi2, holi3];
const holiImages = [holi1, holi2, holi3];
...
export default function KaarwaanOrphanageVisits() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 overflow-x-hidden">
      ...
      {/* --- HERO SECTION --- */}
      <section className="relative w-full min-h-[85vh] lg:min-h-[90vh] flex flex-col items-center justify-center pt-20 sm:pt-24 lg:pt-28 pb-12 sm:pb-16 px-4 sm:px-6">
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute top-[-10%] left-[-10%] w-[60vw] sm:w-[40vw] h-[60vw] sm:h-[40vw] bg-orange-200/40 rounded-full mix-blend-multiply blur-[80px] sm:blur-[100px]"></div>
          <div className="absolute bottom-[-10%] right-[-10%] w-[60vw] sm:w-[40vw] h-[60vw] sm:h-[40vw] bg-rose-200/40 rounded-full mix-blend-multiply blur-[80px] sm:blur-[100px]"></div>
        </div>
        ...
        <div className="relative z-10 max-w-6xl mx-auto w-full flex flex-col items-center text-center">
          ...
          >
            <div className="inline-flex items-center gap-2 bg-white/70 backdrop-blur-md px-4 sm:px-5 py-1.5 sm:py-2 rounded-full text-orange-600 font-semibold text-xs sm:text-sm shadow-sm mb-4 sm:mb-6 border border-white/60">
              <Users className="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0" />
              <span>Kaarwaa.N - Youth for Change</span>
            </div>
            <h1 className="text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-extrabold text-slate-900 mb-4 sm:mb-6 tracking-tight leading-tight">
              A Journey of <br/>
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-500 via-rose-500 to-purple-600">
                Love & Belonging.
              </span>
            </h1>
            <p className="text-sm sm:text-lg lg:text-xl text-slate-600 max-w-2xl mx-auto mb-6 sm:mb-8 leading-relaxed">
              We are Kaarwaa.N. We spend our weekends bridging the gap for children in local orphanages through education, mentorship, and unforgettable festive celebrations.
            </p>
            
            <div className="flex flex-col sm:flex-row justify-center gap-3 sm:gap-4 w-full sm:w-auto">
              <button className="w-full sm:w-auto bg-slate-900 hover:bg-slate-800 text-white px-7 sm:px-8 py-3.5 sm:py-4 rounded-full font-bold text-sm sm:text-base lg:text-lg shadow-xl shadow-slate-900/20 active:scale-95 transition-all">
                Volunteer With Kaarwaa.N
              </button>
            </div>
          </motion.div>
          ...
          <motion.div 
            ...
            animate={{ opacity: 1, scale: 1 }}
            transition={{ delay: 0.3, duration: 0.8 }}
            className="w-full max-w-5xl h-[260px] sm:h-[380px] md:h-[460px] lg:h-[500px] relative rounded-2xl sm:rounded-3xl lg:rounded-[2.5rem] overflow-hidden shadow-xl sm:shadow-2xl shadow-rose-900/10 border-2 sm:border-4 border-white"
          >
            <AnimatePresence mode="wait">
              <motion.img
                key={currentSlide}
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-black/10 to-transparent flex flex-col justify-end p-4 sm:p-8">
              <div className="flex justify-center gap-2 sm:gap-3">
                {heroImages.map((_, idx) => (
                  <button 
                    ...
                    onClick={() => setCurrentSlide(idx)}
                    className={`h-1.5 sm:h-2 rounded-full transition-all duration-500 ${currentSlide === idx ? 'w-6 sm:w-8 bg-white' : 'w-1.5 sm:w-2 bg-white/50 hover:bg-white/80'}`}
                  />
                ))}
              </div>
            </div>
      ...
      {/* 1. Holi Theme */}
      <section className="py-14 sm:py-20 lg:py-24 relative overflow-hidden bg-gradient-to-br from-pink-50 via-purple-50 to-yellow-50">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">
            ...
            <motion.div 
              ...
              className="text-left"
            >
              <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-tr from-pink-500 to-yellow-500 rounded-xl sm:rounded-2xl flex items-center justify-center mb-4 sm:mb-6 shadow-lg shadow-pink-500/30 transform -rotate-6">
                <Droplet className="w-6 h-6 sm:w-8 sm:h-8 text-white" />
              </div>
              <h2 className="text-2xl sm:text-4xl lg:text-5xl font-extrabold text-slate-900 mb-3 sm:mb-6 tracking-tight leading-tight">
                Rang-E-Kaarwaa.N:<br/>Holi at the Shelter
              </h2>
              ...
              <ul className="space-y-2.5 sm:space-y-4 mb-6 sm:mb-8">
                {['Safe, Organic Colors for the Kids', 'Gujiya & Festive Feasts', 'Water Balloon Fights & Dance'].map((item, i) => (
                  <li key={i} className="flex items-center gap-2.5 sm:gap-3 text-slate-700 font-medium text-xs sm:text-sm lg:text-base">
            ...
            <motion.div 
              ...
              whileInView={{ opacity: 1, x: 0 }} 
              viewport={{ once: true }}
              className="grid grid-cols-2 grid-rows-2 gap-3 sm:gap-4 h-[340px] sm:h-[420px] lg:h-[480px] relative"
            >
              <div className="absolute -bottom-4 -left-4 w-28 h-28 bg-yellow-400 rounded-full mix-blend-multiply filter blur-2xl opacity-70 z-0 pointer-events-none"></div>
              <div className="absolute -top-4 -right-4 w-28 h-28 bg-pink-500 rounded-full mix-blend-multiply filter blur-2xl opacity-70 z-0 pointer-events-none"></div>
              
              <img 
                src={holiImages[0]} 
                loading="lazy"
                className="col-span-2 row-span-1 w-full h-full object-cover rounded-2xl sm:rounded-3xl shadow-md relative z-10" 
                alt="Holi Colors" 
              />
              <img 
                src={holiImages[1]} 
                loading="lazy"
                className="col-span-1 row-span-1 w-full h-full object-cover rounded-2xl sm:rounded-3xl shadow-md relative z-10" 
                alt="Kids playing Holi" 
              />
              <img 
                src={holiImages[2]} 
                loading="lazy"
      ...
      <section className="py-14 sm:py-20 lg:py-24 relative bg-blue-50/50">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">
            ...
            <motion.div 
              ...
              className="text-left"
            >
              <div className="w-12 h-12 sm:w-16 sm:h-16 bg-blue-600 rounded-xl sm:rounded-2xl flex items-center justify-center mb-4 sm:mb-6 shadow-lg shadow-blue-600/30">
                <Lightbulb className="w-6 h-6 sm:w-8 sm:h-8 text-white" />
              </div>
              <h2 className="text-2xl sm:text-4xl lg:text-5xl font-extrabold text-slate-900 mb-3 sm:mb-6 tracking-tight leading-tight">
                Building Futures:<br/>Kaarwaa.N Pathshala
              </h2>
              <p className="text-sm sm:text-base lg:text-lg text-slate-600 leading-relaxed mb-6 sm:mb-8">
                Beyond the festivities, consistency is our promise. Our dedicated teaching wing visits the orphanage every weekend to provide foundational academic support, helping the children dream bigger and achieve more.
              </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                <div className="bg-white p-3.5 sm:p-4 rounded-xl sm:rounded-2xl shadow-sm border border-slate-100 flex items-start gap-3">
                  <BookOpen className="w-5 h-5 text-blue-500 mt-0.5 shrink-0" />
                  <div>
                    <h4 className="font-bold text-slate-800 text-sm sm:text-base">Academics</h4>
                    <p className="text-xs sm:text-sm text-slate-500">Math, English & Science</p>
                  </div>
                </div>
                <div className="bg-white p-3.5 sm:p-4 rounded-xl sm:rounded-2xl shadow-sm border border-slate-100 flex items-start gap-3">
                  <PenTool className="w-5 h-5 text-purple-500 mt-0.5 shrink-0" />
                  <div>
                    <h4 className="font-bold text-slate-800 text-sm sm:text-base">Skill Building</h4>
                    <p className="text-xs sm:text-sm text-slate-500">Arts, Crafts & Expression</p>
                  </div>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div 
              ...
              viewport={{ once: true }}
              className="relative h-[280px] sm:h-[380px] md:h-[460px] lg:h-[500px] rounded-2xl sm:rounded-3xl lg:rounded-[2.5rem] overflow-hidden shadow-xl sm:shadow-2xl border-2 sm:border-4 border-white mt-4 lg:mt-0"
            >
              <img 
                src={childstudying} 
                loading="lazy"
                className="w-full h-full object-cover" 
                alt="Learning Workshop at Orphanage" 
              />
            </motion.div>
          </div>
        </div>
