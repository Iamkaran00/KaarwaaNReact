...
const AboutKaarwaan = () => {
  return (
    <div className="bg-slate-50 min-h-screen font-sans text-slate-800 overflow-x-hidden">
      ...
      {/* --- HERO SECTION --- */}
      <section className="relative bg-emerald-900 text-white py-14 sm:py-20 lg:py-24 overflow-hidden">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          <h2 className="text-amber-400 font-bold tracking-widest uppercase mb-3 sm:mb-4 text-xs sm:text-sm">
            Est. 2012 • JEC Jabalpur
          </h2>
          <h1 className="text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-extrabold mb-4 sm:mb-6 tracking-tight">
            KAARWAA.N...
          </h1>
          <p className="text-base sm:text-xl md:text-2xl font-light max-w-3xl mx-auto leading-relaxed opacity-90 px-2">
            "A journey towards social reform."
          </p>
          <div className="mt-6 sm:mt-8 w-16 sm:w-24 h-1 bg-orange-500 mx-auto rounded-full"></div>
        </div>
      </section>
        ...
        <div className="flex flex-col md:flex-row items-center gap-8 sm:gap-12 lg:gap-16">
          ...
          <div className="w-full md:w-1/2">
            <div className="relative max-w-md md:max-w-none mx-auto">
              <div className="absolute -top-3 -left-3 sm:-top-4 sm:-left-4 w-16 sm:w-24 h-16 sm:h-24 bg-orange-100 rounded-full z-0 pointer-events-none"></div>
              <div className="absolute -bottom-3 -right-3 sm:-bottom-4 sm:-right-4 w-20 sm:w-32 h-20 sm:h-32 bg-emerald-100 rounded-full z-0 pointer-events-none"></div>
              <img 
                src={team} 
                alt="Students learning" 
                loading="lazy"
                className="relative z-10 rounded-2xl shadow-xl w-full object-cover h-64 sm:h-80 md:h-[380px] lg:h-[420px]"
              />
            </div>
          </div>
          ...
          <div className="w-full md:w-1/2 space-y-4 sm:space-y-6 text-left">
            <h3 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-emerald-900 tracking-tight">
              Why We Give Back
            </h3>
            <p className="text-slate-600 leading-relaxed text-sm sm:text-base lg:text-lg">
              Every person dreams of becoming their best self. But as youth, we realize that our ambition is hollow if it doesn't lift others up. 
            </p>
            <blockquote className="border-l-4 border-orange-500 pl-4 italic text-slate-700 font-medium my-3 sm:my-4 text-sm sm:text-base leading-relaxed bg-orange-50/50 py-2 rounded-r-lg">
              "What good is the glory of life if not used for the upliftment of others?"
            </blockquote>
      ...
      {/* --- CORE MISSION --- */}
      <section className="bg-white py-12 sm:py-16 lg:py-20 shadow-inner border-y border-slate-100">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="max-w-4xl mx-auto">
            <span className="inline-block bg-emerald-100 text-emerald-800 px-3.5 sm:px-4 py-1 rounded-full text-xs sm:text-sm font-semibold tracking-wide uppercase">
              Our Core Mission
            </span>
            <h3 className="text-2xl sm:text-3xl lg:text-4xl font-bold mt-3 sm:mt-4 mb-4 sm:mb-6 text-slate-800 tracking-tight">
              Educating the Future
            </h3>
            <p className="text-slate-600 mb-8 sm:mb-12 text-sm sm:text-base lg:text-lg max-w-2xl mx-auto leading-relaxed">
              The primary aim of Kaarwaa.N is to educate underprivileged students and prepare them for competitive entrance exams, opening doors to quality education at institutions like:
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
              {[
                ...
                <div key={index} className="bg-slate-50 border border-slate-200/80 p-5 sm:p-6 rounded-2xl hover:shadow-lg transition-all duration-300 group">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 bg-orange-500 text-white rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4 font-bold text-lg sm:text-xl shadow-md group-hover:scale-110 transition-transform">
                    {index + 1}
                  </div>
                  <h4 className="font-bold text-slate-800 text-sm sm:text-base">{school}</h4>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="py-12 sm:py-16 lg:py-20 container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-10 sm:mb-16">
          <h3 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-slate-900 tracking-tight">
            Spreading Smiles Beyond Books
          </h3>
          <p className="text-slate-500 text-xs sm:text-sm sm:text-base mt-2">
            Our social reform extends to six key domains.
          </p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6 lg:gap-8">
          {/* Card 1 */}
          <div className="bg-white p-6 sm:p-8 rounded-2xl sm:rounded-3xl shadow-sm hover:shadow-xl transition-all duration-300 border-t-4 border-emerald-500 border-x border-b border-slate-100 group">
            <Sprout className="w-8 h-8 sm:w-10 sm:h-10 text-emerald-500 mb-3 sm:mb-4 group-hover:scale-110 transition-transform shrink-0" />
            <h4 className="text-lg sm:text-xl font-bold mb-2 text-slate-900">Plantation Drives</h4>
            <p className="text-slate-600 text-xs sm:text-sm leading-relaxed">
              Changing the world starts with saving the planet. We actively organize tree plantation drives to create a greener future.
            </p>
          </div>

          {/* Card 2 */}
          <div className="bg-white p-6 sm:p-8 rounded-2xl sm:rounded-3xl shadow-sm hover:shadow-xl transition-all duration-300 border-t-4 border-orange-500 border-x border-b border-slate-100 group">
            <Users className="w-8 h-8 sm:w-10 sm:h-10 text-orange-500 mb-3 sm:mb-4 group-hover:scale-110 transition-transform shrink-0" />
            <h4 className="text-lg sm:text-xl font-bold mb-2 text-slate-900">Orphanage Visits</h4>
            <p className="text-slate-600 text-xs sm:text-sm leading-relaxed">
              Making children feel loved and valued. We spend quality time spreading joy to those who need it most.
            </p>
          </div>

          {/* Card 3 */}
          <div className="bg-white p-6 sm:p-8 rounded-2xl sm:rounded-3xl shadow-sm hover:shadow-xl transition-all duration-300 border-t-4 border-emerald-500 border-x border-b border-slate-100 group">
            <BookOpen className="w-8 h-8 sm:w-10 sm:h-10 text-emerald-500 mb-3 sm:mb-4 group-hover:scale-110 transition-transform shrink-0" />
            <h4 className="text-lg sm:text-xl font-bold mb-2 text-slate-900">Teaching Kids</h4>
            <p className="text-slate-600 text-xs sm:text-sm leading-relaxed">
              Beyond exam prep, we provide basic education to poor children to pave the way for better standards of living.
            </p>
          </div>

          {/* Card 4 */}
          <div className="bg-white p-6 sm:p-8 rounded-2xl sm:rounded-3xl shadow-sm hover:shadow-xl transition-all duration-300 border-t-4 border-orange-500 border-x border-b border-slate-100 group">
            <Gift className="w-8 h-8 sm:w-10 sm:h-10 text-orange-500 mb-3 sm:mb-4 group-hover:scale-110 transition-transform shrink-0" />
            <h4 className="text-lg sm:text-xl font-bold mb-2 text-slate-900">Cloth Distribution</h4>
            <p className="text-slate-600 text-xs sm:text-sm leading-relaxed">
              Providing essentials to the needy, ensuring dignity and comfort for the underprivileged sections of society.
            </p>
          </div>

          {/* Card 5 */}
          <div className="bg-white p-6 sm:p-8 rounded-2xl sm:rounded-3xl shadow-sm hover:shadow-xl transition-all duration-300 border-t-4 border-emerald-500 border-x border-b border-slate-100 group">
            <Heart className="w-8 h-8 sm:w-10 sm:h-10 text-emerald-500 mb-3 sm:mb-4 group-hover:scale-110 transition-transform shrink-0" />
            <h4 className="text-lg sm:text-xl font-bold mb-2 text-slate-900">Awareness Programs</h4>
            <p className="text-slate-600 text-xs sm:text-sm leading-relaxed">
              Raising informed individuals by spreading awareness about critical social and environmental domains.
            </p>
          </div>

          {/* Card 6 */}
          <div className="bg-white p-6 sm:p-8 rounded-2xl sm:rounded-3xl shadow-sm hover:shadow-xl transition-all duration-300 border-t-4 border-orange-500 border-x border-b border-slate-100 group">
            <MapPin className="w-8 h-8 sm:w-10 sm:h-10 text-orange-500 mb-3 sm:mb-4 group-hover:scale-110 transition-transform shrink-0" />
            <h4 className="text-lg sm:text-xl font-bold mb-2 text-slate-900">Study Material</h4>
            <p className="text-slate-600 text-xs sm:text-sm leading-relaxed">
              Donating books and stationery to ensure that a lack of resources never stops a child from learning.
            </p>
          </div>
        </div>
      </section>
      ...
      {/* --- CTA SECTION --- */}
      <section className="py-12 sm:py-16 text-center bg-emerald-50/50 border-t border-emerald-100/60">
        <div className="container mx-auto px-4 sm:px-6 max-w-3xl">
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-slate-900 mb-3 sm:mb-4 tracking-tight">
            Be Part of <span className="text-emerald-700">Kaarwaa.N...</span>
          </h2>
          <p className="text-slate-600 text-xs sm:text-sm lg:text-base mb-6 sm:mb-8 leading-relaxed">
            Changing the world starts with raising informed individuals. Join us in this journey of spreading smiles and creating futures.
          </p>
          <Link to="/donate" className="inline-block w-full sm:w-auto">
            <button className="w-full sm:w-auto bg-orange-500 hover:bg-orange-600 active:scale-95 text-white font-bold py-3 sm:py-3.5 px-8 rounded-full shadow-lg shadow-orange-500/25 transition-all text-sm sm:text-base">
              Get Involved
            </button>
          </Link>
        </div>
