...
const FAQItem = ({ faq }) => {
  ...
  return (
    <div className="border-b border-gray-100 last:border-0">
      <button
        className="w-full py-4 sm:py-5 flex justify-between items-center text-left focus:outline-none transition-colors group"
        onClick={() => setIsOpen(!isOpen)}
        aria-expanded={isOpen}
      >
        <span className="text-base sm:text-lg font-semibold text-gray-800 group-hover:text-blue-600 transition-colors pr-3">
          {faq.question}
        </span>
        <span className="p-1.5 rounded-full bg-gray-50 group-hover:bg-blue-50 text-gray-500 group-hover:text-blue-600 transition-colors shrink-0">
          {isOpen ? (
      ...
      >
        <p className="text-xs sm:text-sm md:text-base text-gray-600 pr-2 sm:pr-8 leading-relaxed">
          {faq.answer}
        </p>
      </div>
    </div>
    ...
    <section className="bg-gray-50 py-10 sm:py-16 lg:py-20">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-8 sm:mb-12">
          ...
          <p className="mt-2 sm:mt-4 text-xs sm:text-sm md:text-base text-gray-500 max-w-xl mx-auto leading-relaxed">
            Everything you need to know about joining and volunteering with Kaarwaa.N
          </p>
        </div>
        
        <div className="bg-white shadow-sm sm:shadow-md rounded-xl sm:rounded-2xl border border-gray-100/80 px-4 sm:px-6 lg:px-8 py-1 sm:py-2">
          {faqs.map((faq, index) => (
            <FAQItem key={index} faq={faq} />
          ))}
        </div>
