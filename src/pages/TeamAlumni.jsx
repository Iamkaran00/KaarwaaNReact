...
//       { name: "Ms. Devanshi Jain", role: "Technical Head", img: '/img/mbr2022_26/dvnsi2.jpeg' },
...
const batches = [
  {
    ...
    members: [
      ...
      { name: "Mr. Devesh Pandey", role: "Secretary", img: "/img/mbr2023_27/DeveshP.jpeg" },
      { name: "Mr. Nihal Jain", role: "Treasurer", img: "/img/mbr2023_27/NihalJ.jpeg" },
      { name: "Mr. Arjun Kurmi", role: "Managment Head", img:"/img/mbr2023_27/arjunkurmi.jpg" },
      { name: "Mr. Harshit Soni", role: "Media Head", img: "/img/mbr2023_27/HarshitS.jpeg" },
      { name: "Ms. Jaishree Shukla", role: "Teaching Head", img: null },
      { name: "Ms. Pranjal Singh", role: "Teaching Head", img: '/img/mbr2023_27/Pranjal.jpeg' },
      { name: "Mr. Piyush Dwivedi", role: "Teaching Head", img: "/img/mbr2023_27/PiyushD.webp" },
...
function AvatarFallback({ female }) {
  return (
    <svg className="w-16 h-16 sm:w-20 sm:h-20" viewBox="0 0 56 56" fill="none" xmlns="http://www.w3.org/2000/svg">
      <circle cx="28" cy="28" r="28" fill={female ? "#FCE7F3" : "#E0F2FE"} />
      <circle cx="28" cy="20" r="9" fill={female ? "#F472B6" : "#7DD3FC"} />
      {female ? (
        <>
...
function MemberCard({ member }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200/80 flex flex-col overflow-hidden transition-all duration-300 hover:-translate-y-1.5 hover:shadow-xl hover:border-slate-300 shadow-sm cursor-default">
      {/* Aspect Ratio container */}
      <div className={`w-full aspect-square flex items-center justify-center overflow-hidden border-b border-slate-100 ${
        hasImage ? "bg-slate-50" : female ? "bg-pink-50/50" : "bg-sky-50/50"
      }`}>
        {hasImage ? (
          <img
            src={member.img}
      ...
      <div className="p-4 sm:p-5 flex flex-col flex-grow justify-between gap-3 text-left">
        <div>
          <div className="text-[11px] sm:text-xs text-slate-400 font-medium mb-0.5">
            {prefix}
          </div>
          <div className="font-bold text-sm sm:text-base lg:text-[17px] text-slate-900 leading-snug line-clamp-2">
            {cleanName}
          </div>
        </div>

        <div>
          <span className={`${theme.bg} ${theme.text} ${theme.border} border text-[11px] sm:text-xs font-semibold px-2.5 sm:px-3 py-1 rounded-md inline-block`}>
            {member.role}
          </span>
        </div>
      </div>
...
function BatchSection({ batch, defaultOpen }) {
  ...
  return (
    <div className="mb-4 sm:mb-6 shadow-sm rounded-xl sm:rounded-2xl overflow-hidden border border-slate-200/80">
      <button
        onClick={() => setOpen((o) => !o)}
        className={`w-full p-4 sm:p-5 lg:p-6 flex items-center justify-between transition-all duration-200 text-left ${
          isCurrentBatch ? "bg-slate-900 text-white" : "bg-white text-slate-900 hover:bg-slate-50"
        }`}
      >
        <div className="flex items-center gap-2.5 sm:gap-4 flex-wrap">
          ...
          {isCurrentBatch && (
            <span className="bg-blue-600 text-white text-[10px] sm:text-xs font-semibold px-2.5 py-0.5 rounded-full uppercase tracking-wider shadow-sm">
              Current
            </span>
          )}
        </div>
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          <span className={`text-xs sm:text-sm font-medium ${isCurrentBatch ? "text-slate-400" : "text-slate-500"}`}>
            {batch.members.length} members
          </span>
          <ChevronDown
            size={18}
            className={`transition-transform duration-300 ${open ? "rotate-180" : "rotate-0"} ${
              isCurrentBatch ? "text-white" : "text-slate-600"
            }`}
          />
        </div>
      </button>
      ...
      {open && (
        <div className="p-4 sm:p-6 lg:p-8 bg-slate-50/60 border-t border-slate-200/80">
...
export default function TeamAlumni() {
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 font-sans overflow-x-hidden pt-20 sm:pt-24 lg:pt-28">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        
        {/* Header & Metrics */}
        <div className="text-center mb-8 sm:mb-12">
          <h1 className="text-3xl sm:text-5xl lg:text-6xl font-extrabold text-slate-900 mb-3 sm:mb-4 tracking-tight">
            Our Team & Alumni
          </h1>
          <p className="text-slate-600 text-sm sm:text-base lg:text-lg max-w-2xl mx-auto mb-6 sm:mb-8 leading-relaxed">
            Celebrating {totalMembers}+ members across {batches.length} batches of impact and leadership since 2011.
          </p>
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-4 max-w-lg mx-auto">
            {[
              ...
              <div key={i} className="bg-white border border-slate-200/80 shadow-sm rounded-xl sm:rounded-2xl p-3 sm:p-5 text-center">
                <div className="font-extrabold text-lg sm:text-2xl lg:text-3xl text-slate-900">{s.value}</div>
                <div className="text-[11px] sm:text-xs text-slate-500 font-medium mt-0.5 sm:mt-1">{s.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Search & Filter Bar */}
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mb-8 sm:mb-10 bg-white p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-200/80 shadow-sm">
          <div className="relative flex-1">
            ...
            <input
              ...
              value={search}
              onChange={e => setSearch(e.target.value)}
              className="w-full pl-10 pr-4 py-2.5 sm:py-3 border border-slate-200 rounded-lg sm:rounded-xl text-xs sm:text-sm md:text-base outline-none bg-slate-50 text-slate-900 focus:border-blue-500 focus:bg-white transition-all"
            />
          </div>
          <select
            value={filterRole}
            onChange={e => setFilterRole(e.target.value)}
            className="py-2.5 sm:py-3 px-4 border border-slate-200 rounded-lg sm:rounded-xl text-xs sm:text-sm md:text-base bg-slate-50 text-slate-900 cursor-pointer outline-none focus:border-blue-500 focus:bg-white transition-all sm:min-w-[200px]"
          >
            {allRoles.map(r => <option key={r} value={r}>{r}</option>)}
          </select>
        </div>
        ...
        {/* Batch Lists */}
        {filteredBatches.length === 0 ? (
          <div className="text-center py-16 sm:py-20 px-4 text-slate-500 text-sm sm:text-base bg-white rounded-2xl border border-slate-200/80">
            No members found matching your search criteria.
          </div>
        ) : (
          filteredBatches.map((batch, i) => (
        ...
        {/* Missing Name Note */}
        <div className="text-center mt-10 sm:mt-14 p-4 sm:p-6 bg-white rounded-xl sm:rounded-2xl border border-slate-200/80 shadow-sm max-w-2xl mx-auto">
          <p className="text-slate-600 text-xs sm:text-sm leading-relaxed flex items-center justify-center gap-2 flex-wrap">
            <span>We apologize if your name is missing or information is incorrect. Contact our web team at:</span>
            <a href="tel:7440682926" className="inline-flex items-center gap-1 font-bold text-slate-900 hover:text-blue-600 transition-colors">
              <PhoneCall size={14} className="text-blue-600" />
              <span>7440682926</span>
            </a>
          </p>
        </div>
